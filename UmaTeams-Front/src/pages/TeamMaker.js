            ...
            {teams.length > 0 ? (
              teams.map((team) => (
                <TeamCard key={team.id}>
                  <h4>{team.teamName} (ID: {team.id})</h4>
                  {Array.isArray(team.teamMembers) && team.teamMembers.length > 0 && (
                    <ul>
                      {team.teamMembers.map((member) => (
                        <li key={member.id}>{member.umaName} (ID: {member.umaId})</li>
                      ))}
                    </ul>
                  )}
                </TeamCard>
              ...
              <p style={{ textAlign: "center", marginTop: "20px" }}>No hay equipos creados.</p>
            )}
          </div>
        </TrCardGrid>
